...
\section{AFNS model}
The $\Q$-dynamics of the state variable $X_{t}$ is 3 -factor Gaussian

$$
\d  X_{t}=\kappa^{\Q}\left(\theta^{\Q}-X_{t}\right) \d  t+\Sigma \d  W_{t}^{\Q} .
$$
...
$$
b^{\prime}(\tau)=-\left(\kappa^{\Q}\right)^{\prime} b(\tau)+\rho_{1}, \quad b(0)=0
$$
...
X_{t}^{2} \\
X_{t}^{3}
\end{array}\right)\right] \d  t+\Sigma\left(\begin{array}{l}
\d  W_{t}^{1, \Q} \\
\d  W_{t}^{2, \Q} \\
\d  W_{t}^{3, \Q}
\end{array}\right) .
...
Proof. The ODE $b^{\prime}(\tau)=-\left(\kappa^{\Q}\right)^{\prime} b(\tau)+\rho_{1}$ (here $\rho_{1}=(1,1,0)^{\prime}$ ) is equivalent to
...
The above only considers the $\Q$-dynamics of $X_{t}$. For the $\P$-dynamics we use the essentially affine models from Duffee. In the Gaussian case we have $\lambda_{t}=\lambda_{1}+\lambda_{2} X_{t}$, so the $\P$-dynamics become

$$
\d  X_{t}=\kappa^{\P}\left(\theta^{\P}-X_{t}\right) \d  t+\Sigma \d  W_{t}^{\P}
$$

Although the $\Q$-dynamics was restricted (e.g. zero terms in $\kappa^{\Q}$ ) we can choose ( $\lambda_{1}, \lambda_{2}$ ) appropriately to get any $\kappa^{\P}$ and $\theta^{\P}$.
