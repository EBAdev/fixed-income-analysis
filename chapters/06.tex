...
$$
P_{\tau}\left(x_{t}\right)=\mathbb{E}_{t}^{\mathbb{Q}}\left[\mathrm{e}^{-\int_{t}^{t+\tau} r\left(x_{s}\right) \mathrm{d} s}\right]
$$

The dynamics of the state variable are

$$
\mathrm{d} x_{t}=\mu^{\mathbb{Q}}\left(x_{t}\right) \mathrm{d} t+\Sigma\left(x_{t}\right) \mathrm{d} B_{t}^{\mathbb{Q}}=\left(\mu^{\mathbb{Q}}\left(x_{t}\right)+\Sigma\left(x_{t}\right) \lambda\left(x_{t}\right)\right) \mathrm{d} t+\Sigma\left(x_{t}\right) \mathrm{d} B_{t}^{\mathbb{P}}
$$
...
\mathrm{~d} x_{t} & =\kappa^{\mathrm{Q}}\left(\theta^{\mathrm{Q}}-x_{t}\right) \mathrm{d} t+\Sigma \mathrm{d} B_{t}^{\mathrm{Q}} \\
\lambda_{t} & =\lambda_{a}+\Lambda_{b} x_{t}
\end{aligned}
$$

where $\Phi, \kappa^{\mathrm{Q}}, \Sigma, \Lambda_{b}$ are $2 \times 2$ and $\theta^{\mathrm{Q}}, \lambda_{a}$ are $2 \times 1$ matrices. (Note: $x_{t}$ is a Gaussian model, i.e. in $\left.\mathbb{A}_{0}(2)\right)$. The yield of maturity $\tau$ is
...
where $\rho$ is a constant and $\Phi, \kappa^{\mathbb{Q}}, \Sigma, \Lambda_{b}$ are $2 \times 2$ and $\gamma, \theta^{\mathbb{Q}}, \lambda_{a}$ are $2 \times 1$ matrices.\\
Letting $P(t, x)$ be the price of a zero-coupon bond maturing at $T$ Munk inplies

$$
\frac{\partial P}{\partial t}+\left(\kappa^{\mathbb{Q}}\left(\theta^{\mathbb{Q}}-x_{t}\right)\right)^{\prime} \frac{\partial P}{\partial x}+\frac{1}{2} \operatorname{tr}\left(\frac{\partial^{2} P}{\partial x^{2}} \Sigma \Sigma^{\prime}\right)-\max \{s(x), 0\}=0
$$
...
\begin{aligned}
r_{t} & =\max \left\{s_{t}, 0\right\} \\
\mathrm{d} s_{t} & =\kappa^{\mathbb{P}}\left(\theta^{\mathbb{P}}-s_{t}\right) \mathrm{d} t+\sigma \mathrm{d} B_{t}^{\mathbb{P}} \\
...
Affine models: The 2 -factor CIR model ( $\mathbb{A}_{2}(2)$ ) with $\mathbb{Q}$-dynamics
...
\kappa_{11}^{\mathrm{Q}} & \kappa_{12}^{\mathrm{Q}} \\
\kappa_{21}^{\mathrm{Q}} & \kappa_{22}^{\mathrm{Q}}
...
0 & \sqrt{x_{2 t}}
\end{array}\right]\left[\begin{array}{l}
\mathrm{d} B_{1 t}^{\mathrm{Q}} \\
\mathrm{~d} B_{2 t}^{\mathrm{Q}}
\end{array}\right],
$$
...
$$
\left[\kappa^{\mathbb{Q}} \theta^{\mathbb{Q}}\right]_{i}>0, \quad \kappa_{i i}^{\mathbb{Q}}>0, \quad i=1,2, \quad \text { and } \quad \kappa_{12}^{\mathbb{Q}}, \kappa_{21}^{\mathbb{Q}}<0
$$
