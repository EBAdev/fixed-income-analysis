...
real-world measure $\mathbb{P}$). The short rate dynamics under the risk-neutral measure
$\mathbb{Q}$ is
\[
...
\end{theorem*}
\begin{proof}
 We will prove that the candidate for a solution is in fact a solution and then use the uniqueness
...
& \implies \quad  a^{\prime}(T-t)-\hat{\varphi} b(T-t)+\frac{1}{2} \delta_{1}
b(T-t)^{2}+\left(b^{\prime}(T-t)+\hat{\kappa} b(T-t)+\frac{1}{2} \delta_{2} b(T-t)^{2}-1\right)
r=0,
\end{align*}
...
\[
\mathrm{d} r_{t}=\left[\kappa\left(\theta-r_{t}\right)-\beta \lambda(r, t)\right] \mathrm{d} t+\beta
\mathrm{d} z_{t}^{\Q}=\kappa\left(\hat{\theta}-r_{t}\right) \mathrm{d} t+\beta \mathrm{d}
z_{t}^{\Q}
\]
...
parameters
\[
\hat{\varphi}=\kappa \hat{\theta}, \qquad \hat{\kappa}=\kappa, \qquad
...
\[
b(\tau)=\frac{1}{\kappa}\left(1-\mathrm{e}^{-\kappa \tau}\right) \quad \text { and } \quad a(\tau)=\left(\hat{\theta}-\frac{\beta^{2}}{2 \kappa^{2}}\right)(\tau-b(\tau))+\frac{\beta^{2}}{4 \kappa} b(\tau)^{2}
\]
...
\[
\mathrm{d} r_{t}=\hat{\kappa}\left(\hat{\theta}-r_{t}\right) \mathrm{d} t+\beta \sqrt{r_{t}} \mathrm{~d} z_{t}^{\Q}
\]
